...
# متن‌های قابل‌تغییر ربات: (کلید settings بدون پیشوند, عنوان تو پنل, مقدار پیش‌فرض)
BOT_TEXTS = [
    ("welcome_intro", "متن خوش‌آمدگویی (بعد از /start)", "به ربات فروش خوش آمدید"),
    ("topup_prompt", "پیام درخواست مبلغ شارژ کیف پول", "💠 مبلغ مورد نظر برای شارژ کیف پول را به تومان وارد کنید."),
    ("support_prompt", "پیام شروع ارسال تیکت پشتیبانی", "🎧 پیام خود را برای پشتیبانی ارسال کنید:"),
]
...
<div class="sidebar">
  ...
  <a href="{{ url_for('appearance_page') }}" class="{{ 'active' if active=='appearance' else '' }}">🎨 متن‌ها و ظاهر ربات</a>
...
def appearance_page():
    ...
    texts_html = ""
    for key, label, default in BOT_TEXTS:
        value = get_setting("text_" + key, default)
        texts_html += f"""
        <label>{label}</label>
        <textarea name="text_{key}" rows="2">{value}</textarea>
        """
