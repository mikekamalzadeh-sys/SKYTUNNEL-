...
def appearance_page():
    ...
    texts_html = ""
    # اگه main_menu/buy_menu نیاز به اضافه‌شدن خط DNS داشته باشن، تغییرشون رو
    # اینجا جمع می‌کنیم و آخر سر با هم (نه یکی‌یکی) ذخیره می‌کنیم.
    pending_dns_fix = {}
    for group_title, items in BOT_TEXT_GROUPS:
        ...
        for key, label, default, rows in items:
            value = all_settings.get("text_" + key, default)
            ...
            if key in ("main_menu", "buy_menu") and "DNS بازی" not in value:
                value = value.rstrip() + "\n🎮 فقط DNS بازی: <b>{dns_price}</b> تومان/گیگ"
                pending_dns_fix["text_" + key] = value
            texts_html += f"""
            <label>{label}</label>
            <textarea name="text_{key}" rows="{rows}">{value}</textarea>
            """
    for k, v in pending_dns_fix.items():
        set_setting(k, v)
