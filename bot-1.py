...
def build_config_note(chat_id, username, price):
    ...
    تاریخ و ساعت دقیق (با ثانیه) ساخت، آیدی عددی و یوزرنیم خریدار، و مبلغی
    که برای این کانفیگ پرداخت کرده."""
...
def action_buy_menu(chat_id):
    ...
    type_kb = {
        'inline_keyboard': [
            [{'text': ct['wireguard']['label'], 'callback_data': 'buytype_wireguard'}],
            [{'text': ct['config']['label'], 'callback_data': 'buytype_config'}],
            [{'text': ct['both']['label'], 'callback_data': 'buytype_both'}],
            [{'text': ct['openvpn']['label'], 'callback_data': 'buytype_openvpn'}],
        ]
    }
...
def action_support_start(chat_id):
    ...
    send_message(chat_id, get_bot_text('support_prompt', DEFAULT_SUPPORT_PROMPT))
