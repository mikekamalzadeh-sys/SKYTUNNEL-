...
# دستورهایی که معادل مستقیم یکی از دکمه‌های کیبورد اصلی هستن؛ با نگاشت به همون
# متن دکمه، از تکرار منطق پردازش جلوگیری می‌شه (همون elif های موجود اجراشون می‌کنن)
COMMAND_TO_BUTTON = {
    '/buy': '🛍 خرید کانفیگ',
    '/test': '🧪 تست رایگان',
    '/wallet': '💠 شارژ کیف پول',
        ...
        if updates.get('ok') and updates.get('result'):
            for update in updates['result']:
                ...
                # ------------------------------------------------------------------
                    ...
                    # دستورهایی مثل /buy که معادل یکی از دکمه‌های منو هستن، به همون
                    # متن دکمه تبدیل می‌شن تا از منطق پردازش موجود استفاده کنن
                    if text in COMMAND_TO_BUTTON:
                        text = COMMAND_TO_BUTTON[text]
                        ...
                        try:
                            ...
                            if extra_days <= 0:
                                ...
                                if not cfg:
                                    ...
                                    price_per_gb = CONFIG_TYPES.get(ctype, CONFIG_TYPES['both'])['price_per_gb']
                        ...
                        if not new_label:
                            ...
                            if not cfg:
                                ...
                                if res['success']:
                                    ...
                                    send_message(chat_id, '✅ نام کانفیگ به «' + new_label + '» تغییر کرد.')
                                else:
