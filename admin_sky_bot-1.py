...
# ---------------------------------------------------------------------------
# دیتابیس (همون دیتابیس Turso که ربات فروش می‌سازه)
# ---------------------------------------------------------------------------
...
def delete_webhook():
    try:
        res = requests.post(BASE_URL + "/deleteWebhook", json={"drop_pending_updates": False}, timeout=10)
...
def get_updates(offset=None):
    ...
    try:
        ...
        data = res.json()
        if not data.get("ok"):
            print("⚠️ خطای getUpdates (بات ادمین):", data)
        return data
    except Exception as e:
        print("⚠️ استثنا در getUpdates (بات ادمین):", e)
