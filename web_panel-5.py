...
BOT_TEXT_GROUPS = [
    ("🏠 پیام /start و منوی اصلی (همون تصویری که می‌بینید)", [
        ...
        ("main_menu", "قالب کامل پیام منوی اصلی — placeholder های مجاز: "
         "{welcome_intro} {wireguard_price} {config_price} {both_price} {openvpn_price} {dns_price} {wallet} {divider}",
         "✨ <b>{welcome_intro}</b>\n{divider}\n"
         ...
         "💳 موجودی کیف پول: <b>{wallet}</b> تومان", 9),
    ]),
    ("🛍 خرید کانفیگ", [
        ("buy_menu", "قالب پیام «انتخاب نوع سرویس» — placeholder ها: "
         "{wireguard_price} {config_price} {both_price} {openvpn_price} {dns_price} {wallet} {divider}",
         ...
         "📱 فقط OpenVPN: <b>{openvpn_price}</b> تومان/گیگ\n"
         "🎮 فقط DNS بازی: <b>{dns_price}</b> تومان/گیگ\n\n"
         "💳 موجودی فعلی: <b>{wallet}</b> تومان", 9),
...
def get_ordered_keys(setting_key, default_keys):
    ...
    تو کد هیچ‌وقت باعث گم‌شدنش تو پنل نشه."""
...
<style>
  :root {
    ...
    --muted: #8b96b3;
    --accent: #35e7c6;
    --accent-rgb: 53,231,198;
    --accent-ink: #06342c;
    --accent-dim: rgba(53,231,198,0.14);
    --warn: #f5b94d;
    --warn-dim: rgba(245,185,77,0.14);
    --danger: #ff6b6b;
  ...
  * { box-sizing: border-box; }
  body {
    ...
    background:
      radial-gradient(ellipse 900px 420px at 50% -60px, rgba(53,231,198,0.10), transparent 65%),
      var(--bg);
    color: var(--ink);
  }
  ...
  .avatar {
    ...
    font-size: 13px; font-family: var(--mono); font-weight: 700; color: var(--accent);
    box-shadow: 0 0 0 3px var(--accent-dim);
  }
  .topbar h1 { font-size: 16.5px; margin: 0; letter-spacing: .2px; }
  .topbar .sub { font-size: 11.5px; color: var(--muted); display: flex; align-items: center; gap: 6px; margin-top: 2px; }
  .status-dot {
    width: 7px; height: 7px; border-radius: 50%; background: var(--accent);
    box-shadow: 0 0 0 0 rgba(53,231,198,0.55);
    animation: pulse 2.2s infinite;
  }
  @keyframes pulse {
    0% { box-shadow: 0 0 0 0 rgba(53,231,198,0.55); }
    70% { box-shadow: 0 0 0 6px rgba(53,231,198,0); }
    100% { box-shadow: 0 0 0 0 rgba(53,231,198,0); }
  }
  .icon-btn {
    width: 38px; height: 38px; border-radius: 11px; background: var(--surface);
    border: 1px solid var(--border); display: flex; align-items: center;
    justify-content: center; text-decoration: none; color: var(--muted); font-size: 15px;
  }
  ...
  .card .value { font-size: 21px; font-weight: 700; font-family: var(--mono); letter-spacing: -.3px; }
  ...
  .banner span:last-child { color: var(--muted); font-size: 12px; font-family: var(--mono); font-weight: 400; }
  ...
  th { background: var(--surface-2); color: var(--muted); font-weight: 600; font-size: 11.5px; border-bottom: 1px solid var(--accent-dim); }
  tr:last-child td { border-bottom: none; }
  tbody tr:hover { background: var(--surface-2); }

  input[type=text], input[type=number], input[type=password], textarea, select {
    background: var(--surface); border: 1.5px solid var(--border); color: var(--ink);
    border-radius: 10px; padding: 11px 14px; font-size: 14px; width: 100%;
  }
  input:focus, textarea:focus, select:focus { outline: none; border-color: var(--accent); }
  textarea { min-height: 100px; resize: vertical; font-family: inherit; }
  label { display: block; margin: 12px 0 6px; font-size: 12.5px; color: var(--muted); font-weight: 600; }
  ...
  .reorder-arrows button { padding: 5px 11px; font-size: 11px; line-height: 1.4; border-radius: 8px; }
  ...
  button.secondary, .btn.secondary { background: transparent; color: var(--ink); border: 1.5px solid var(--border); }
  button.danger, .btn.danger { background: transparent; color: var(--danger); border: 1.5px solid var(--danger-dim); }
  ...
  .flash { background: var(--accent-dim); color: var(--accent); padding: 11px 16px; border-radius: 10px; margin-bottom: 14px; font-size: 13.5px; font-weight: 600; border: 1px solid var(--accent-dim); }
  ...
  .bottom-nav a {
    ...
    color: var(--muted); text-decoration: none; font-size: 11px; flex: 1; position: relative;
  }
  ...
  /* ---------- drawer ---------- */
  .drawer-overlay {
    display: none; position: fixed; inset: 0; background: rgba(5,8,15,0.65); z-index: 50;
  }
  ...
  .drawer {
    ...
    border-radius: 20px 20px 0 0; padding: 10px 18px 26px; max-height: 82vh; overflow-y: auto;
  }
  .drawer .handle { width: 36px; height: 4px; background: var(--border); border-radius: 4px; margin: 8px auto 14px; }
  .drawer h3 { font-size: 12px; margin: 18px 0 8px; color: var(--muted); text-transform: none; }
  ...
  .login-box {
    background: var(--surface); border: 1px solid var(--border); padding: 34px 28px;
    border-radius: 18px; width: 320px; max-width: 100%;
    opacity: 0; transform: translateY(14px) scale(0.98); pointer-events: none;
    transition: opacity 0.5s ease 0.1s, transform 0.5s cubic-bezier(.2,.8,.3,1) 0.1s;
  }
  .login-wrap.on .login-box { opacity: 1; transform: translateY(0) scale(1); pointer-events: auto; }
  .login-box h2 { margin: 6px 0 22px; text-align: center; font-size: 17px; }

  /* ---------- pull-cord lamp ---------- */
  .lamp-wrap { display: flex; flex-direction: column; align-items: center; user-select: none; -webkit-user-select: none; margin-bottom: 8px; }
  #glow { opacity: 0; transition: opacity 0.5s ease; }
  .login-wrap.on #glow { opacity: 1; }
  #bulb { fill: var(--border); transition: fill 0.4s ease; }
  .login-wrap.on #bulb { fill: var(--accent); }
  #filament { stroke: var(--muted); transition: stroke 0.4s ease; }
  .login-wrap.on #filament { stroke: var(--accent-ink); }
  #cordBead { cursor: grab; }
  .pull-hint { margin-top: 12px; font-size: 12.5px; color: var(--muted); letter-spacing: 0.02em; text-align: center; transition: opacity 0.4s ease; }
  .login-wrap.on .pull-hint { opacity: 0; }
</style>
</head>
<body>
{{ body|safe }}
...
LOGIN_HTML = """
<div class="login-wrap" id="loginScene">
  <div class="lamp-wrap">
    <svg viewBox="0 0 220 260" width="170" height="200">
      <circle id="glow" cx="110" cy="95" r="90" fill="url(#glowGrad)"/>
      <defs>
        <radialGradient id="glowGrad" cx="50%" cy="50%" r="50%">
          <stop offset="0%" stop-color="rgba(var(--accent-rgb),0.55)"/>
          <stop offset="100%" stop-color="rgba(var(--accent-rgb),0)"/>
        </radialGradient>
      </defs>
      <line x1="110" y1="0" x2="110" y2="30" stroke="var(--border)" stroke-width="3"/>
      <path d="M60 70 L160 70 L145 30 L75 30 Z" fill="var(--surface-2)" stroke="var(--border)" stroke-width="2"/>
      <ellipse cx="110" cy="70" rx="50" ry="8" fill="var(--bg)"/>
      <circle id="bulb" cx="110" cy="98" r="20"/>
      <line id="filament" x1="103" y1="98" x2="117" y2="98" stroke-width="2"/>
      <line x1="110" y1="118" x2="110" y2="128" stroke="var(--muted)" stroke-width="3"/>
      <g id="cordHit">
        <line id="cord-line" x1="110" y1="128" x2="110" y2="190" stroke="var(--muted)" stroke-width="2"/>
        <circle id="cordBead" cx="110" cy="196" r="9" fill="var(--accent)"/>
      </g>
    </svg>
    <div class="pull-hint" id="pullHint">برای ورود، دستگیره را بکش ↓</div>
  </div>

  <form method="post" class="login-box">
    <h2>📡 ورود به پنل</h2>
    {% if error %}<div class="flash" style="background:var(--danger-dim);color:var(--danger);border:1px solid var(--danger-dim);">{{ error }}</div>{% endif %}
...
(function(){
  ...
  var bead = document.getElementById('cordBead');
  var cordLine = document.getElementById('cord-line');
  if(!scene || !bead) return;

  var REST_Y = 196, PULL_THRESHOLD = 40, MAX_STRETCH = 60;
  var isOn = false, dragging = false, startClientY = 0;

  function setBeadY(y){
    bead.setAttribute('cy', y);
    cordLine.setAttribute('y2', y - 6);
  }
  function springBack(){
    bead.style.transition = 'cy 0.35s cubic-bezier(.34,1.6,.4,1)';
    cordLine.style.transition = 'y2 0.35s cubic-bezier(.34,1.6,.4,1)';
    setBeadY(REST_Y);
    setTimeout(function(){ bead.style.transition = ''; cordLine.style.transition = ''; }, 360);
  }
  ...
  function onPointerMove(e){
    ...
    if(delta < 0) delta = 0;
    if(delta > MAX_STRETCH) delta = MAX_STRETCH;
    setBeadY(REST_Y + delta);
  }
  function onPointerUp(){
    ...
    var currentY = parseFloat(bead.getAttribute('cy'));
    if((currentY - REST_Y) >= PULL_THRESHOLD){
      isOn = !isOn;
      scene.classList.toggle('on', isOn);
    }
    springBack();
  }
  bead.addEventListener('mousedown', onPointerDown);
  bead.addEventListener('touchstart', onPointerDown, {passive:true});
  bead.setAttribute('tabindex','0');
  bead.setAttribute('role','button');
  bead.setAttribute('aria-label','بکش تا فرم ورود نمایش داده شود');
  bead.addEventListener('keydown', function(e){
    if(e.key === 'Enter' || e.key === ' '){
      isOn = !isOn;
      scene.classList.toggle('on', isOn);
      setBeadY(REST_Y + MAX_STRETCH);
      springBack();
    }
  });

  {% if error %}
  // اگر ورود قبلی خطا داشته، مستقیم فرم رو نشون بده تا کاربر مجبور نشه دوباره بکشه
...
def appearance_page():
    ...
    for group_title, items in BOT_TEXT_GROUPS:
        ...
        for key, label, default, rows in items:
            ...
            # main_menu و buy_menu از قبل (قبل از اضافه‌شدن DNS) توسط شما
            # سفارشی و ذخیره شده بودن، پس خط قیمت DNS رو نداشتن. این‌جا،
            # همون لحظه‌ای که صفحه باز می‌شه، اگه ببینیم خط DNS نیست، هم به
            # خودِ متنی که نشون داده می‌شه اضافه‌ش می‌کنیم، هم مستقیم تو
            # دیتابیس ذخیره‌ش می‌کنیم — تا هم تو پنل هم تو بات همیشه باشه،
            # بدون این‌که لازم باشه شما دستی چیزی تایپ کنید.
            if key in ("main_menu", "buy_menu") and "DNS بازی" not in value:
                value = value.rstrip() + "\n🎮 فقط DNS بازی: <b>{dns_price}</b> تومان/گیگ"
    ...
    main_order = get_ordered_keys("menu_order_main", MAIN_MENU_KEYS)
    ...
    for key in main_order:
        value = get_setting("btn_" + key, main_defaults[key])
        buttons_html += f"""
    ...
    buytype_order = get_ordered_keys("menu_order_buytypes", CONFIG_TYPE_KEYS)
    ...
    for key in buytype_order:
        value = get_setting("ctype_label_" + key, buytype_defaults[key])
        buytype_buttons_html += f"""
        <div class="reorder-row" data-key="{key}">
          <div>
            <label>{buytype_labels[key]}</label>
            <input type="text" name="ctype_label_{key}" value="{value}" required>
          </div>
          <div class="reorder-arrows">
            <button type="button" class="secondary" onclick="moveRow(this,-1)" title="بالا">▲</button>
            <button type="button" class="secondary" onclick="moveRow(this,1)" title="پایین">▼</button>
